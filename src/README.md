...
3. **最小依赖** —— Node 22 自带 `fetch`，仅新增 `tsx`（运行 TS）与 `fast-xml-parser`（解析 RSS）。
...
├── lib/                 # 共享库：types / fetch / dedupe / verify
...
3. 无 RSS 的信源登记为 `type: 'html'`、`enabled: false` 占位，待 HTML 采集实现后启用。
